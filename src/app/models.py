...
participants = Table(
    ...
    Column(
        "hangout_id", UUID(as_uuid=True), ForeignKey("hangouts.id"), primary_key=True
    ),
...
expense_shared_by = Table(
    "expense_shared_by",
    Base.metadata,
    Column("user_id", UUID(as_uuid=True), ForeignKey("users.id"), primary_key=True),
    Column(
        "expense_id", UUID(as_uuid=True), ForeignKey("expenses.id"), primary_key=True
    ),
)
...
class User(Base, AuditMixinAt):
    ...
    comments = relationship(
        "Comment", primaryjoin="User.id==Comment.author_id", back_populates="author"
    )
...
class Comment(Base, AuditMixinAt, AuditMixinBy):
    ...
    author = relationship(
        "User", primaryjoin="User.id==Comment.author_id", back_populates="comments"
    )
