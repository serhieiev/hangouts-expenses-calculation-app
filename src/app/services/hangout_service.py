...
async def create_hangout(db: AsyncSession, hangout: HangoutCreate, creator_id: UUID) -> Hangout:
    new_hangout = Hangout(
...
async def get_all_hangouts(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Hangout]:
    result = await db.execute(select(Hangout).offset(skip).limit(limit))
    return result.scalars().all()

async def get_hangout_details(db: AsyncSession, hangout_id: UUID) -> Optional[HangoutDetails]:
    ...
    organizer_result = await db.execute(select(User).filter(User.id == hangout.organizer_id))
