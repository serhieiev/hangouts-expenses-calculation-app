...
async def create_new_hangout(
    hangout: hangout.HangoutCreate,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    return await hangout_service.create_hangout(
        db=db, hangout=hangout, creator_id=current_user.id
    )
